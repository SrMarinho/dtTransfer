...
app = typer.Typer(help="Verifica erros nos logs")
...
@app.command()
def check_errors(
    date: Annotated[str, typer.Option("--date", help="Data no formato YYYY-MM-DD")] = None,
    days_ago: Annotated[int, typer.Option("--days-ago", help="Quantos dias atras (ex: 1 = ontem)")] = None,
    since: Annotated[str, typer.Option("--since", help="Hora de inicio do intervalo HH:MM")] = None,
    until: Annotated[str, typer.Option("--until", help="Hora de fim do intervalo HH:MM")] = None,
    detailed: Annotated[bool, typer.Option("--detailed", help="Exibe mensagens de erro completas")] = False,
):
    """Verifica erros de tabelas nos logs do dataReplicator."""
    if date:
    ...
    if not table_errors and not generic_errors:
        ...
        typer.secho(f"[{dt.strftime('%d/%m/%Y')} {since_s}->{until_s}] Nenhum erro encontrado.", fg=typer.colors.GREEN)
        return
