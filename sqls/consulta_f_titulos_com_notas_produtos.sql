SELECT
	...
	COALESCE(NFP.VLRDFA, 0) AS DIFERENCIAL_ICMS,
	NFP.VLRIRF AS IRRF,
	NFP.VLRPIS AS PIS_RECUPERAR,
	NFP.VLRCOR AS COFINS_RECUPERAR,
	NFP.VLRCSL AS CSLL,
	NFP.VLROUR AS OUTRAS_RETENCOES,
	...
	COALESCE(NFP.NUMOCP, 0) AS NUM_ORDEM_COMPRA,
	COALESCE(NFP.NUMCTR, 0) AS NUM_CONTRATO,
	COALESCE(TO_CHAR(OCP.DATEMI,'DD/MM/YYYY'),'00/00/0000')  AS DATA_EMISSAO_OC,
	NFP.NUMNFC AS NOTA_FISCAL,
	NFP.SEQIPC AS NFE_SEQ_PRODUTO,
	NFP.CODPRO AS PRODUTO,
	NFP.CODFAM AS FAM_PRODUTO,
	FAM.DESFAM AS DESCR_FAM_PROD,
	NFP.CPLIPC AS DESCR_PRODUTO,
...
FROM SAPIENS_PROD.E501TCP T
   ...
   INNER JOIN SAPIENS_PROD.E440IPC NFP 
	       ON NFP.CODEMP = NDG.CODEMP 
	      AND NFP.CODFIL = NDG.CODFIL 
	      AND NFP.CODFOR = NDG.CODFOR 
	      AND NFP.NUMNFC = NDG.NUMNFC
	      AND NFP.CODSNF = NDG.CODSNF 
   INNER JOIN SAPIENS_PROD.E001TNS TNF
   		   ON TNF.CODEMP = NFP.CODEMP 
   		  AND TNF.CODTNS = NFP.TNSPRO 
   INNER JOIN SAPIENS_PROD.E012FAM FAM 
    	   ON FAM.CODEMP = NFP.CODEMP
    	  AND FAM.CODFAM = NFP.CODFAM
    LEFT JOIN SAPIENS_PROD.E420OCP OCP ---ordem de compra
   		   ON OCP.CODEMP = NFP.CODEMP 
   		  AND OCP.CODFIL = NFP.CODFIL
   		  AND OCP.NUMOCP = NFP.NUMOCP
    ...
    LEFT JOIN SAPIENS_PROD.R999USU UA
   	...
   	LEFT JOIN SAPIENS_PROD.E460CTR CTR ---contrato
		   ON CTR.CODEMP = NFP.CODEMP 
		  AND CTR.CODFIL = NFP.CODFIL 
		  AND CTR.NUMCTR = NFP.NUMCTR
   ...
   INNER JOIN SAPIENS_PROD.E440RAT RNFP
   		   ON RNFP.CODEMP = NFP.CODEMP 
   		  AND RNFP.CODFIL = NFP.CODFIL 
   		  AND RNFP.CODFOR = NFP.CODFOR 
   		  AND RNFP.NUMNFC = NFP.NUMNFC
   		  AND RNFP.CODSNF = NFP.CODSNF
   		  AND RNFP.SEQIPC = NFP.SEQIPC
   INNER JOIN SAPIENS_PROD.E044CCU CCU
   		   ON CCU.CODEMP = RNFP.CODEMP
   		  AND CCU.CODCCU = RNFP.CODCCU
   INNER JOIN SAPIENS_PROD.E045PLA PLA
           ON PLA.CODEMP = RNFP.CODEMP
          AND PLA.CTARED = RNFP.CTARED
